...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        return [[
            [self.start_of_word] + [self.char2id[c] for c in word] + [self.end_of_word] for word in sent
        ] for sent in sents]
        ### END YOUR CODE
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        ###     previous parts
        sents_char_indices = self.words2charindices(sents)
        padded_sents = pad_sents_char(sents_char_indices, self.char2id['<pad>'])

        sents_var_raw = torch.tensor(padded_sents, device=device)
        sents_var = torch.transpose(sents_var_raw, 0, 1).contiguous()
        return sents_var
        ### END YOUR CODE
